#!/usr/bin/env python3
"""CLI for Integrated Daily Strategy."""
...
def main():
    p = argparse.ArgumentParser(description="Integrated Daily Strategy")
    ...
    runner = IntegratedStrategyRunner(paths)
    strategy = runner.run(args.date, args.bankroll, args.windows_left)

    out = r/"reports"/"generated"
    runner.write_json(strategy, str(out/"integrated_daily_strategy.json"))
    runner.write_markdown(strategy, str(out/"integrated_daily_strategy.md"))

    pools = strategy.integrated_candidate_pools.get("pools", [])
    total = sum(p.get("candidate_count", 0) for p in pools)
    print(f"Date: {strategy.current_date} | State: {strategy.bankroll_state} | Pool candidates: {total}", file=sys.stderr)
    for p in pools:
        print(f"  {p['bucket']}: {p['candidate_count']} candidates | budget: {p['bucket_strategy_budget']} CNY", file=sys.stderr)
